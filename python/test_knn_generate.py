...
def create_suffix():
    ...
    print("  sycl::buffer<Ti, 1> bhid(sycl::range<1>(inset.size() * args.get_knn().k));")
    print("  sycl::buffer<Tf, 1> bhpq(sycl::range<1>(inset.size() * args.get_knn().k));")
    ...
    print("  });")
    print()
    print("  std::vector<Ti> vid(inset.size() * args.get_knn().k);")
    print("  std::vector<Tf> vpq(inset.size() * args.get_knn().k);")
    print("  auto ahid = bhid.get_host_access();")
    print("  auto ahpq = bhpq.get_host_access();")
    print("  std::copy_n(ahid.get_pointer(), inset.size() * args.get_knn().k, vid.begin());")
    print("  std::copy_n(ahpq.get_pointer(), inset.size() * args.get_knn().k, vpq.begin());")
    print()
