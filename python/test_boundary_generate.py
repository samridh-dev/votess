...
def create_prefix():
    ...
    print('  ')
    print('  sycl::queue q;')
    print('  sycl::buffer<T> bcycle(cycle.data(), sycl::range<1>(cycle.size()));')
    ...
    print('    auto aR = bR.template ')
    print('    get_access<sycl::access::mode::read_write>(h);')
    print('    auto ahead = bhead.template ')
    print('    get_access<sycl::access::mode::read_write>(h);')
    print('    sycl::local_accessor<T> ')
    print('    lcycle(sycl::range<1>(cycle.size()), h);')
    print('    ')
    print('    sycl::nd_range<1> ndRange(1, 1);')
    print('    h.parallel_for(ndRange, [=](sycl::nd_item<1> item) {')
    print('')
    print('      for (size_t i = 0; i < cycle.size(); i++) {')
    print('        lcycle[i] = cycle[i];')
    print('      }')
    print('')
    print('      boundary::compute<size_t, T>(lcycle, dr_offs, dr_size, ')
    print('                                   ahead[0], aR, r_offs, r_size);')
    print('')
    print('      for (size_t i = 0; i < cycle.size(); i++) {')
    print('        cycle[i] = lcycle[i];')
    print('      }')
    print('')
    print('    });')
    print('  }).wait();')
